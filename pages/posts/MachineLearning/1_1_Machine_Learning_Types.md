...
1. **数据集 (data set)** 由一系列 **样本 (sample)** 所组成的集合．样本由多种 **属性 (attribute)** 组成，每种属性都有相应的值．其中*样本可以被理解成向量，属性可以被理解成维度，属性值就是向量在这个维度上的分量*．例如西瓜数据集 $\mathcal {X}$ 里面的某个西瓜 $\boldsymbol{x}_{i}$ 就是一个样本：$$\boldsymbol x_{i} = (0.9, 0.4, 0.7)$$比如这里第一个属性是色泽，第二个属性是敲声，第三个属性是花纹．
